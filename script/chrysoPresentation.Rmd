...
# Nettoyer les géométries, c'est supprimer les geometrycollection, n'avoir que des polygones.
...
toc()
tic()
res <- sapply(1:4, function(i){anneeUnique(lzone4,i)})
toc()
```
...
enregistrerAnneeUnique <- function(df, num) {
  ...
  Y <- df [df$an !=  an [num],]
  nomZone <-  X$groupe [1]
  X <- st_union(X) %>% st_set_precision(1000000) %>% st_make_valid()
  Y <- st_union(Y) %>% st_set_precision(1000000) %>% st_make_valid()
