...
- dissoudre les petites parcelles (st_join) sur chacune des zones et des années, il faut vérifier que cela allège
...
a <- aggregate(rpg [,"an"], list (nb = lengths (st_intersects(rpg, aggAlsace))> 0), length)
str(rpg)
```
...
### Organiser la donnée par zone et année

```{r}
# on éclate les données par zone puis par an et on efface les frontières entre parcelles.
# 
lzone <- split(inter, inter$groupe)
#df <-  lzone[[4]]
recupPoly <- function (sfc) {
...
eclater <- function (num) {
 ...
 # on unit toutes les géométries
  ...
  lpoly <- lapply(lunion, recupPoly)
}
lzone1 <- eclater (1)
lzone2 <- eclater (2)
lzone3 <- eclater (3)
lzone4 <- eclater (4)
# 116 / 34.7 / 50.5 et 2.6 M°
...
## Contrôle validité des géométries après l'intersection zone, données

### Cas des GEOMETRY COLLECTION 

Cartographie des GEOMETRY COLLECTION

```{r}
# type de géométrie, on retient que les polygones / multipolygones
# elimination des geometries pouvant poser pb
table(st_geometry_type(inter))
# 407 sur 275 M géometries....
anomalie <- inter [st_geometry_type(inter) == "GEOMETRYCOLLECTION",]
ghostLayer(aggAlsace [aggAlsace$groupe == 3,])
plot(anomalie$geom, add = TRUE)
library(units)
anomalie$aire <- round(drop_units(st_area(anomalie$geom)/10000),0)
plot(anomalie$aire)
grosseAnomalie <- anomalie [anomalie$aire > 25,]
ghostLayer(aggAlsace [aggAlsace$groupe == 3,])
par(mfrow = c(2,3))
for (i in 1:5) {
  plot(grosseAnomalie$geom [i])
  labelLayer(grosseAnomalie, txt = "aire", cex = 1.2,halo = 3)
}
petiteAnomalie <- anomalie [anomalie$aire < 0.5,]
library(mapview)
mapview(petiteAnomalie)
st_write(grosseAnomalie, "../dataS/chrysomele.gpkg", "panomalie", delete_layer = TRUE, quiet = TRUE)
st_write(petiteAnomalie, "../dataS/chrysomele.gpkg", "ganomalie", delete_layer = TRUE, quiet = TRUE)
# Ces "geometry collection" ne se visionnent pas donc on les enlève.
...
```{r}
res <- anneeUnique(lzone2,1)
sapply(lzone2, anneeUnique)
```


### Récupération et filtrage de la donnée




#### Vision des géométries
